<?php
// Incluimos los archivos de lógica
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validar selects contra los IDs reales de la BD
    if (empty($datos['provincia']) || !array_key_exists($datos['provincia'], $provincias)) {
        $errores['provincia'] = "Seleccione una provincia válida.";
    }

    if (empty($datos['sede']) || !array_key_exists($datos['sede'], $sedes)) {
        $errores['sede'] = "Seleccione una sede válida.";
    }

    if (empty($datos['departamento']) || !array_key_exists($datos['departamento'], $departamentos)) {
        $errores['departamento'] = "Seleccione un departamento válido.";
    }

    // 3. Procesar: Guardar en BD si no hay errores
    if (empty($errores)) {
        try {
            guardarEmpleado($datos); // <--- ESTO ES LO QUE FALTABA
            $mostrarResumen = true;
            $mensajeExito = "¡Empleado registrado correctamente en la Base de Datos!";
        } catch (Exception $e) {
            $mensajeErrorBD = "Error al guardar: " . $e->getMessage();
        }
    }
}
?>
...
<div class="contenedor">
    ...
    <?php if ($mensajeErrorBD): ?>
        <div class="mensaje-error" style="background-color: #f8d7da; color: #721c24; padding: 10px; margin-bottom: 15px; border-radius: 4px;">
    ...
    <?php if ($mostrarResumen): ?>
        ...
        <div class="resumen">
            ...
            <ul>
                ...
                <!-- Mostramos el nombre recuperado del array usando el ID -->
    ...
    <?php else: ?>
        ...
        <form action="index.php" method="POST" novalidate>
            ...
            <!-- Desplegables cargados desde BD -->
            <div class="form-group">
